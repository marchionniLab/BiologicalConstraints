...
ROCTrain <- roc(usedTrainGroup, ktspStatsTrainRes$statistics, plot = F, print.thres=thr, print.thres.adj=c(0.01,1.25), print.auc=TRUE, ci = T, print.auc.col="black", levels = c("NoProgression", "Progression"), direction = "<", col="blue", lwd=2, grid=TRUE, main="Mechanistic KTSP performance in the training data")
ROCTrain
...
confusionMatrix(usedTrainPredictionRes, usedTrainGroup, positive = "Progression", mode = "everything")
...
EMTAB_Out_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(EMTAB_Out_MechPerformance, file = "./Objs/KTSP/EMTAB_Out_MechPerformance.rda")
...
#################################################################
### Plot Curve
png("./Figs/KTSP/CompareAUCggplot_EMTABOut.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic K-TSP (EMTAB Out)"
...
save(basicplot_KTSP_EMTABOut, file = "./Objs/KTSP/BasicPlot_KTSP_EMTABout.rda")
