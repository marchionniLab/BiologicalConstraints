...
load("./Objs/progressionDataGood_pmidOut.rda")
...
### Threshold
thr <- coords(roc(usedTrainGroup, ktspStatsTrainRes$statistics, levels = c("NoProgression", "Progression"), direction = "<"), "best")["threshold"]
thr

### Print ROC curve local maximas
coords(roc(usedTrainGroup, ktspStatsTrainRes$statistics, levels = c("NoProgression", "Progression"), direction = "<"), "local maximas")

### Plot Curve: note that you must reorder the levels!!!
### ("good" goes first, "bad" goes second, the opposite of confusionMatrix)
ROCTrain <- roc(usedTrainGroup, ktspStatsTrainRes$statistics, plot = F, print.thres=thr, ci = T, print.auc=TRUE, print.auc.col="black", levels = c("NoProgression", "Progression"), direction = "<", col="blue", lwd=2, grid=TRUE, main="Mechanistic KTSP performance in the training data")
ROCTrain
...
confusionTrain <- confusionMatrix(usedTrainPredictionRes, usedTrainGroup, positive = "Progression", mode = "everything")
confusionTrain
...
## Plot curve
ROCTest <- roc(usedTestGroup, ktspStatsTestRes$statistics, plot = F, print.auc=TRUE, ci = T, print.auc.col="black", levels = c("NoProgression", "Progression"), direction = "<", col="blue", lwd=2, grid=TRUE, main= "Mechanistic KTSP using TF_MiR Gns")
ROCTest
...
confusionTest <- confusionMatrix(usedTestPredictionRes, usedTestGroup, positive = "Progression", mode = "everything")
confusionTest
...
PMID_Out_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(PMID_Out_MechPerformance, file = "./Objs/KTSP/PMID_Out_MechPerformance.rda")
...
forLegend_KTSP <- apply(rbind(
  ci(roc(usedTrainGroup, ktspStatsTrainRes$statistics, levels = c("NoProgression", "Progression"), direction = "<")),
  ci(roc(usedTestGroup, ktspStatsTestRes$statistics, levels = c("NoProgression", "Progression"), direction = "<")),
  ci(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("NoProgression", "Progression"), direction = "<")),
  ci(roc(usedTestGroup, ktspStatsTestUnRes$statistics, levels = c("NoProgression", "Progression"), direction = "<"))
),  1, function(x) {
...
### Plot
basicplot_KTSP_PmidOut
### Close device
dev.off()

save(basicplot_KTSP_PmidOut, file = "./Objs/KTSP/BasicPlot_KTSP_Pmidout.rda")
