...
### Cross-study validation : PMID159-- out
###############################################################################
...
## Threshold
thr <- coords(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("NoProgression", "Progression"), direction = "<"), "best")["threshold"]
thr

## Print ROC curve local maximas
coords(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("NoProgression", "Progression"), direction = "<"), "local maximas")

### Plot Curve: reorder the levels ("good" goes first, "bad" goes second, the opposite of confusionMatrix)
ROCTrain <- roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, plot = FALSE, ci = TRUE, print.auc = TRUE, print.auc.col="black", levels = c("NoProgression", "Progression"), direction = "<", col = "blue", lwd = 2, grid = TRUE, main = "Agnostic KTSP ROC Train")
ROCTrain
...
confusionTrain <- confusionMatrix(usedTrainPredictionUnRes, usedTrainGroup, positive = "Progression")
confusionTrain
...
# plot the curve
ROCTest <- roc(usedTestGroup, ktspStatsTestUnRes$statistics, plot = F, ci = T, print.auc=TRUE, print.auc.col="black", levels = c("NoProgression", "Progression"), direction = "<", col= "blue", lwd=2, grid = TRUE, main = "Agnostic KTSP ROC Test")
ROCTest
...
confusionTest <- confusionMatrix(usedTestPredictionUnRes, usedTestGroup, positive = "Progression", mode = "everything")
confusionTest
...
PMID_Out_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(PMID_Out_AgnosticPerformance, file = "./Objs/KTSP/PMID_Out_AgnosticPerformance.rda")
