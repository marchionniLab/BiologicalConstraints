...
# Tunning RF model (to find the best mtry)   ## 500 since OOB error is lower than 100
set.seed(333)
...
set.seed(333)
RF_Agnostic <- randomForest(x =predictor_data, y=target, importance = TRUE, ntree = 1000, mtry = 7, proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
print(RF_Agnostic)
#plot(RF_Agnostic)
...
GSE13507_Out_RF_IndvGenes_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE13507_Out_RF_IndvGenes_AgnosticPerformance, file = "./Objs/RF/GSE13507_Out_RF_IndvGenes_AgnosticPerformance.rda")
