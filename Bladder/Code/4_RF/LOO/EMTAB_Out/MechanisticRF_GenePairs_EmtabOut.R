...
ROCTest <- roc(usedTestGroup, RF_predictions_votes_Mechanistic_OnKTSP[,2], direction = "<", plot = F, print.auc = TRUE, levels = c("NoProgression", "Progression"), col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTest
...
EMTAB_Out_RF_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(EMTAB_Out_RF_MechPerformance, file = "./Objs/RF/EMTAB_Out_RF_MechPerformance.rda")
...
###################
### Plot Curve
png("./Figs/RF/CompareAUCggplot_EmtabOut.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic RF (EMTAB-4321 for validation))"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_RF_EMTABOut <- ggplot(dat_RF, aes(d=Progression, m=RF_sum, color=RF_type,
                                               linetype = RF_type)) +
...
### Plot
basicplot_RF_EMTABOut
### Close device
dev.off()


save(basicplot_RF_EMTABOut, file = "./Objs/RF/BasicPlot_RF_EMTABOut.rda")
