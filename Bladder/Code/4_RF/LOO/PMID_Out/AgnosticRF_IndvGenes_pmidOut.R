...
set.seed(333)
RF_Agnostic <- randomForest(x =predictor_data, y=target, importance = TRUE, ntree = 1000, mtry = 110, proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
print(RF_Agnostic)
#plot(RF_Agnostic)
...
PMID_Out_RF_IndvGenes_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(PMID_Out_RF_IndvGenes_AgnosticPerformance, file = "./Objs/RF/PMID_Out_RF_IndvGenes_AgnosticPerformance.rda")
