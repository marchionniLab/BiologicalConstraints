...
set.seed(333)
RF_Mechanistic_OnKTSP <- randomForest(x =predictor_data, y=usedTrainGroup, importance = TRUE, ntree = 100, mtry = 3 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
...
PMID_Out_RF_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(PMID_Out_RF_MechPerformance, file = "./Objs/RF/PMID_Out_RF_MechPerformance.rda")
...
###################
### Plot Curve
png("./Figs/RF/CompareAUCggplot_pmidOut.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic RF (pmid15930337 for validation))"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_RF_pmidOut <- ggplot(dat_RF, aes(d=Progression, m=RF_sum, color=RF_type,
                                               linetype = RF_type)) +
...
### Plot
basicplot_RF_pmidOut
### Close device
dev.off()


save(basicplot_RF_pmidOut, file = "./Objs/RF/BasicPlot_RF_pmidOut.rda")
