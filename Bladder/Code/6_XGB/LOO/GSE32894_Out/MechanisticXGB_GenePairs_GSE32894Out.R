...
parameters <- list(
  ...
  silent             = 1,                 # default = 02  # Booster Parameters
  ...
  colsample_bytree   = 0.9,           #0.9      # default = 1,   range: (0,1]
...
GSE32894_Out_XGB_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE32894_Out_XGB_MechPerformance, file = "./Objs/XGB/GSE32894_Out_XGB_MechPerformance.rda")
...
###################
### Plot Curve
png("./Figs/XGB/CompareAUCggplot_GSE32894Out.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic XGB (GSE32894 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_XGB_GSE32894Out <- ggplot(dat_XGB, aes(d=Progression, m=XGB_sum, color=XGB_type,
                                             linetype = XGB_type)) +
...
### Plot
basicplot_XGB_GSE32894Out
### Close device
dev.off()

save(basicplot_XGB_GSE32894Out, file = "./Objs/XGB/BasicPlot_XGB_GSE32894Out.rda")
