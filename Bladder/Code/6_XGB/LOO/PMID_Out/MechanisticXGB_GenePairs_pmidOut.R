...
# Mechanistic XGB
# Gene pairs
# Cross-Study validation (pmid out)
...
## Load data
load("./Objs/KTSP/KTSP_STATs_Mechanistic_pmidOut.rda")
...
PMID_Out_XGB_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(PMID_Out_XGB_MechPerformance, file = "./Objs/XGB/PMID_Out_XGB_MechPerformance.rda")
...
# write.csv(importance_matrix, file = "./Objs/XGB/CV_XGB_importanceMatrix_Mechanistic.csv")
# 
# 
# # variable importance plot
# importance_matrix <- importance_matrix[order(importance_matrix$Gain, decreasing = TRUE), ]
# 
# png(filename = "./Figs/XGB/XGB_Importance_Mechanistic_CV.png", width = 3000, height = 2000, res = 300)
...
###################
### Plot Curve
png("./Figs/XGB/CompareAUCggplot_pmidOut.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic XGB (PMID15930337 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_XGB_pmidOut <- ggplot(dat_XGB, aes(d=Progression, m=XGB_sum, color=XGB_type,
                                     linetype = XGB_type)) +
...
### Plot
basicplot_XGB_pmidOut
### Close device
dev.off()

save(basicplot_XGB_pmidOut, file = "./Objs/XGB/BasicPlot_XGB_pmidOut.rda")
