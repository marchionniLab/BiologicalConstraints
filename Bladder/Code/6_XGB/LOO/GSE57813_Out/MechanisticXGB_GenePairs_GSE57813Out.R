...
parameters <- list(
  ...
  colsample_bytree   = 0.8,           #0.8     # default = 1,   range: (0,1]
  colsample_bylevel  = 1,    #1  # default = 1,   range: (0,1]
  lambda             = 0.5,             #0  # default = 1
  alpha              = 0,           # 0    # default = 0
  # Task Parameters
...
GSE57813_Out_XGB_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE57813_Out_XGB_MechPerformance, file = "./Objs/XGB/GSE57813_Out_XGB_MechPerformance.rda")
...
###################
### Plot Curve
png("./Figs/XGB/CompareAUCggplot_GSE57813Out.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic XGB (GSE57813 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_XGB_GSE57813Out <- ggplot(dat_XGB, aes(d=Progression, m=XGB_sum, color=XGB_type,
                                                 linetype = XGB_type)) +
...
### Plot
basicplot_XGB_GSE57813Out
### Close device
dev.off()

save(basicplot_XGB_GSE57813Out, file = "./Objs/XGB/BasicPlot_XGB_GSE57813Out.rda")
