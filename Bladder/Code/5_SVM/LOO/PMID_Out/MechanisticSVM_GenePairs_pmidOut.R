...
Grid <- expand.grid(degree = 3, scale = 0.001, C = 2)
set.seed(333)
...
PMID_Out_SVM_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(PMID_Out_SVM_MechPerformance, file = "./Objs/SVM/PMID_Out_SVM_MechPerformance.rda")
...
###################
### Plot Curve
png("./Figs/SVM/CompareAUCggplot_SVM_pmidOut.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic SVM (PMID15930337 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_SVM_pmidOut <- ggplot(dat_SVM, aes(d=Progression, m=SVM_sum, color=SVM_type,
                                                 linetype = SVM_type)) +
...
### Plot
basicplot_SVM_pmidOut
### Close device
dev.off()

########################

save(basicplot_SVM_pmidOut, file = "./Objs/SVM/BasicPlot_SVM_pmidOut.rda")
