...
Grid <- expand.grid(degree = 1, scale = 0.1, C = 1)
set.seed(333)
...
GSE13507_Out_SVM_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE13507_Out_SVM_MechPerformance, file = "./Objs/SVM/GSE13507_Out_SVM_MechPerformance.rda")
...
###################
### Plot Curve
png("./Figs/SVM/CompareAUCggplot_SVM_GSE13507Out.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic SVM (GSE13507 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_SVM_GSE13507Out <- ggplot(dat_SVM, aes(d=Progression, m=SVM_sum, color=SVM_type,
                                     linetype = SVM_type)) +
...
### Plot
basicplot_SVM_GSE13507Out
### Close device
dev.off()

########################

save(basicplot_SVM_GSE13507Out, file = "./Objs/SVM/BasicPlot_SVM_GSE13507Out.rda")
