...
EMTAB_Out_SVM_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(EMTAB_Out_SVM_MechPerformance, file = "./Objs/SVM/EMTAB_Out_SVM_MechPerformance.rda")
...
###################
### Plot Curve
png("./Figs/SVM/CompareAUCggplot_SVM_EmtabOut.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic SVM (EMTAB-4321 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_SVM_EMTABOut <- ggplot(dat_SVM, aes(d=Progression, m=SVM_sum, color=SVM_type,
                                                 linetype = SVM_type)) +
...
### Plot
basicplot_SVM_EMTABOut
### Close device
dev.off()

########################

save(basicplot_SVM_EMTABOut, file = "./Objs/SVM/BasicPlot_SVM_EMTABOut.rda")
