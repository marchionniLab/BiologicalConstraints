...
Grid <- expand.grid(degree = 1, scale = 0.001, C = 0.5)
set.seed(333)
...
ROCTrain <- roc(usedTrainGroup, train_pred_prob_svmPoly_agnostic[,2], plot = F, ci = T, print.auc=TRUE, levels = c("Sensitive", "Resistant"), direction = "<", col="blue", lwd=2, grid=TRUE, main = "ROC Test SVM Poly (Agnostic)")
ROCTrain

# Best threshold
thr <- coords(roc(usedTrainGroup, train_pred_prob_svmPoly_agnostic[,2], levels = c("Sensitive", "Resistant"), direction = "<"), "best")["threshold"]
thr <- thr$threshold
thr

## Convert predicted probabilities to binary outcome
train_pred_classes_svmPoly_agnostic <- ifelse(train_pred_prob_svmPoly_agnostic[,2] >= thr, "Resistant", "Sensitive")
table(train_pred_classes_svmPoly_agnostic)
# Convert to factor
train_pred_classes_svmPoly_agnostic <- factor(train_pred_classes_svmPoly_agnostic, levels = c("Sensitive", "Resistant"))

## Creat a confusion matrix (the predictions against the true classes)
Confusion_train_svmPoly <- confusionMatrix(train_pred_classes_svmPoly_agnostic, usedTrainGroup, positive = "Resistant")
...
ROCTest <- roc(usedTestGroup, test_pred_prob_svmPoly_agnostic[,2], plot = F, print.auc=TRUE, ci = T, levels = c("Sensitive", "Resistant"), direction = "<", col="blue", lwd=2, grid=TRUE, main = "ROC Test SVM Poly (Agnostic)")
ROCTest

# Predict classes according to the best threshold
test_pred_classes_svmPoly_agnostic <- ifelse(test_pred_prob_svmPoly_agnostic[,2] > thr, "Resistant", "Sensitive")
table(test_pred_classes_svmPoly_agnostic)
# Convert to factor
test_pred_classes_svmPoly_agnostic <- factor(test_pred_classes_svmPoly_agnostic, levels = c("Sensitive", "Resistant"))

## Creat a confusion matrix (the predictions against the true classes)
Confusion_test_svmPoly <- confusionMatrix(test_pred_classes_svmPoly_agnostic, usedTestGroup, positive = "Resistant")
...
GSE32646_Out_SVM_IndvGenes_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE32646_Out_SVM_IndvGenes_AgnosticPerformance, file = "./Objs/SVM/GSE32646_Out_SVM_IndvGenes_AgnosticPerformance.rda")
