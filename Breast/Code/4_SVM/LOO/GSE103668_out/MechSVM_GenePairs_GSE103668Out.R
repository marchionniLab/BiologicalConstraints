...
## Load data
load("./Objs/KTSP/KTSP_STATs_Mechanistic_GSE103668Out.rda")
...
Grid <- expand.grid(degree = 2, scale = 1, C = 0.25)
set.seed(333)
...
ROCTrain <- roc(usedTrainGroup, train_pred_prob_svmPoly_mechanistic_OnKTSP[,2], plot = F, print.auc=TRUE, ci = T, levels = c("Sensitive", "Resistant"), direction = "<", col="blue", lwd=2, grid=TRUE, main = "ROC Test SVM Poly (Mechanistic)")
ROCTrain

# The best threshold
thr <- coords(roc(usedTrainGroup, train_pred_prob_svmPoly_mechanistic_OnKTSP[,2], levels = c("Sensitive", "Resistant"), direction = "<"), "best")["threshold"]
thr <- thr$threshold
thr

# Predict classes according to the best threshold
train_pred_classes_svmPoly_mechanistic_OnKTSP <- ifelse(train_pred_prob_svmPoly_mechanistic_OnKTSP[,2] > thr, "Resistant", "Sensitive")
table(train_pred_classes_svmPoly_mechanistic_OnKTSP)
# Convert to factor
train_pred_classes_svmPoly_mechanistic_OnKTSP <- factor(train_pred_classes_svmPoly_mechanistic_OnKTSP, levels = c("Sensitive", "Resistant"))

## Creat a confusion matrix (the predictions against the true classes)
Confusion_train_svmPoly_OnKTSP <- confusionMatrix(train_pred_classes_svmPoly_mechanistic_OnKTSP, usedTrainGroup, positive = "Resistant")
...
ROCTest <- roc(usedTestGroup, test_pred_prob_svmPoly_mechanistic_OnKTSP[,2], plot = F, print.auc=TRUE, ci = T, levels = c("Sensitive", "Resistant"), direction = "<", col="blue", lwd=2, grid=TRUE, main = "ROC Test SVM Poly (Mechanistic)")
ROCTest

# Predict classes according to the best threshold
test_pred_classes_svmPoly_mechanistic_OnKTSP <- ifelse(test_pred_prob_svmPoly_mechanistic_OnKTSP[,2] > thr, "Resistant", "Sensitive")
table(test_pred_classes_svmPoly_mechanistic_OnKTSP)
# Convert to factor
test_pred_classes_svmPoly_mechanistic_OnKTSP <- factor(test_pred_classes_svmPoly_mechanistic_OnKTSP, levels = c("Sensitive", "Resistant"))

## Creat a confusion matrix (the predictions against the true classes)
Confusion_test_svmPoly_OnKTSP <- confusionMatrix(test_pred_classes_svmPoly_mechanistic_OnKTSP, usedTestGroup, positive = "Resistant")
...
GSE103668_Out_SVM_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE103668_Out_SVM_MechPerformance, file = "./Objs/SVM/GSE103668_Out_SVM_MechPerformance.rda")
