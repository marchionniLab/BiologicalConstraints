...
############################################################################
## Load INC 60 genes
INCgns <- read_xlsx('./data/Ince_ET60_Signature.xlsx', skip = 1)[, 'Gene Symbol']

myTSPs <- t(combn(INCgns$`Gene Symbol`, 2))
...
GSE20194_Out_MechPerformance <- cbind(TrainPerf, TestPerf)
