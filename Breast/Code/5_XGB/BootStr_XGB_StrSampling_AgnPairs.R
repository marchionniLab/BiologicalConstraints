...
load('./Objs/XGB/XGB_MechBootObject_NotchAndMyc.rda')
...
set.seed(333)
bootobjectAgnostic_119 <- boot(data= Data_train_Agnostic, statistic= XGBStrap, R= 1000, parallel = "multicore", ncpus = 15) 
...
### Save boot objects
save(bootobjectMech, bootobjectAgnostic_25, bootobjectAgnostic_50, bootobjectAgnostic_100, bootobjectAgnostic_119, bootobjectAgnostic_250, file = "./Objs/XGB/XGBBootObjects_NotchAndMyc_AgnPairs.rda")
...
## Work with Agnostic bootobject 100 pairs vs mechanistic
...
## Work with Agnostic bootobject 119 pairs vs mechanistic

AUCs_XGB_Agnostic_119 <- bootobjectAgnostic_119$t
colnames(AUCs_XGB_Agnostic_119) <- c("AUC_Train", "AUC_Test", "N_ImportanVariables")

# Calculate the difference and the CI of the difference in the training data
DiffAgnostic_119 <- AUCs_XGB_Agnostic_119[, "AUC_Train"] - AUCs_XGB_Agnostic_119[, "AUC_Test"]
range(DiffAgnostic_119)
quantile(DiffAgnostic_119, c(0.025, 0.975))
#colnames(Diff) <- "Diff"

## Plot the distributions of the AUCs from both methods in the training data
AgnosticAUC_Train_119 <- data.frame(AUC = AUCs_XGB_Agnostic_119[, "AUC_Train"])
...
## Plot the distributions of the AUCs from both methods in the testing data
AgnosticAUC_Test_119 <- data.frame(AUC = AUCs_XGB_Agnostic_119[, "AUC_Test"])
...
####################################
# Load the AUC comparisons from the indivdial genes and combine them with pairs
load("./Objs/XGB/ModelCompareAUC_238.rda")
...
######################
## For the main figure
ModelCompare_XGB <- rbind(ModelCompareAUC_Train_119, ModelCompareAUC_Test_119)
