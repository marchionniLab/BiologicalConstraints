...
sel <- which(AllModelCompare_Breast_DiffNoFeat$algorithm == "K-TSPs" & AllModelCompare_Breast_DiffNoFeat$modelType == "Mechanistic" & AllModelCompare_Breast_DiffNoFeat$NofFeatAgn != "119 Pairs")
AllModelCompare_Breast_DiffNoFeat$NofFeatAgn[sel] <- "119_Pairs"

sel2 <- which(AllModelCompare_Breast_DiffNoFeat$modelType == "Mechanistic" & AllModelCompare_Breast_DiffNoFeat$NofFeatAgn != "119_Pairs")
AllModelCompare_Breast_DiffNoFeat$NofFeatAgn[sel2] <- "119_Pairs"

AllModelCompare_Breast_DiffNoFeat$NofFeatAgn <- factor(AllModelCompare_Breast_DiffNoFeat$NofFeatAgn, levels =  c("25_Pairs", "50_Pairs", "100_Pairs", "119_Pairs", "250_Pairs", "50_Genes", "100_Genes", "200_Genes", '238_Genes', "500_Genes"))
levels(AllModelCompare_Breast_DiffNoFeat$NofFeatAgn) <- c("25 Pairs", "50 Pairs", "100 Pairs", "119 Pairs", "250 Pairs", "50 Genes", "100 Genes", "200 Genes", "238 Genes", "500 Genes")
...
## Density plot
png(filename = "./Figs/Breast_BS_AllModels_Density_DiffNoFeatAndPairs_filt.png", width = 3000, height = 1500, res = 200)
BS_AUC_ModelCompare <- ggplot(AllModelCompare_Breast_DiffNoFeat, aes(x = AUC, y = modelType, fill = NofFeatAgn, height = ..ndensity..)) + 
  ...
  labs(title="Mechanistic (119 pairs) vs agnostic (different N of features/pairs) performance at predicting the response to neoadjuvant chemotherapy in the testing data") + 
  ylab("Model Type") +
  My_Theme +
