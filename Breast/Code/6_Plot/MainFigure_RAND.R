...
load("./Objs/SVM/ModelCompare_SVM_RAND.rda")
...
AllModelCompare_Breast_AgnIndGenes$modelType <- factor(AllModelCompare_Breast_AgnIndGenes$modelType, levels = c("Mechanistic", "Random_genes"))
levels(AllModelCompare_Breast_AgnIndGenes$modelType) <- c("Mechanistic", "Random genes")
############################################################################
...
# levels(AllModelCompare_Breast_AgnPairs$modelType) <- c("Agnostic (top 74 DEGs)", "Agnostic (37 pairs)", "Mechanistic (37 FFLs)")
# 
# 
# ###########################
...
# AllModelCompare_Breast$modelType <- factor(AllModelCompare_Breast$modelType, levels = c("Mechanistic (37 FFLs)", "Agnostic (top 74 DEGs)", "Agnostic (37 pairs)"))
# 
# sel <- which(AllModelCompare_Breast$algorithm == "KTSP" & AllModelCompare_Breast$modelType == "Agnostic (37 pairs)")
...
## Density plot
png(filename = "./Figs/Breast_BS_AllModels_Density_RAND.png", width = 2500, height = 1500, res = 200)
BS_AUC_ModelCompare <- ggplot(AllModelCompare_Breast_AgnIndGenes, aes(x = AUC, y = modelType, fill = modelType, alpha = data_type, height = ..ndensity..)) + 
  ...
  labs(title="Predicting the response to neoadjuvant chemotherapy in TNBC: density of the AUC distribution of the mechanistic models vs models trained on random genes") + 
  ylab("Model Type") +
  My_Theme +
