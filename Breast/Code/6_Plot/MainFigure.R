...
AllModelCompare_Breast_AgnIndGenes$modelType <- factor(AllModelCompare_Breast_AgnIndGenes$modelType, levels = c("Agnostic_DEGs", "Mechanistic"))
levels(AllModelCompare_Breast_AgnIndGenes$modelType) <- c("Agnostic (top 500 DEGs)", "Mechanistic (241 pairs)")
...
AllModelCompare_Breast_AgnPairs$modelType <- factor(AllModelCompare_Breast_AgnPairs$modelType, levels = c("Agnostic_DEGs", "Agnostic_Pairs", "Mechanistic"))
levels(AllModelCompare_Breast_AgnPairs$modelType) <- c("Agnostic (top 500 DEGs)", "Agnostic (250 pairs)", "Mechanistic (241 pairs)")
...
AllModelCompare_Breast$modelType <- factor(AllModelCompare_Breast$modelType, levels = c("Mechanistic (241 pairs)", "Agnostic (top 500 DEGs)", "Agnostic (250 pairs)"))

#sel <- which(AllModelCompare_Breast$algorithm == "KTSP" & AllModelCompare_Breast$modelType == "Agnostic (25 pairs)")
#AllModelCompare_Breast$modelType[sel] <- "Agnostic (top 74 DEGs)"
...
## Density plot
tiff(filename = "./Figs/Breast_BS_AllModels_Density_new.tiff", width = 3000, height = 2000, res = 250)
BS_AUC_ModelCompare <- ggplot(AllModelCompare_Breast, aes(x = AUC, y = modelType, fill = modelType, alpha = data_type, height = ..ndensity..)) + 
  ...
  scale_x_continuous(limits = c(0.55, 1.02), breaks = seq(0.55, 1, by = 0.1), expand = c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
  labs(title="Predicting the response to neoadjuvant chemotherapy in patients with TNBC: density of the AUC distribution of the agnostic and mechanistic models") + 
  ylab("Model Type") +
  My_Theme +
...
png(filename = "./Figs/Breast_BS_AllModels_Npairs.png", width = 3000, height = 1500, res = 200)
BS_AUC_ModelCompare <- ggplot(ModelCompare_KTSP_Npairs, aes(x = `N of output pairs`, y = `N of training features`, fill = `model type`, height = ..ndensity..)) + 
  ...
  scale_y_discrete(expand = c(0, 0)) +
  labs(title="The number of pairs returned by the K-TSPs model in the triple-negative breast cancer case") + 
  ylab("Model Type") +
  My_Theme +
  #scale_fill_manual(values = c("red3", "#78B7C5", "#3B9AB2")) +
  #scale_fill_jco() +
