...
#usedTrainGroup <- ordered(usedTrainGroup, levels=c("Sensitive", "Resistant"))
...
set.seed(333)
RF_Agnostic <- randomForest(x =predictor_data, y=target, importance = TRUE, ntree = 1000, mtry = 115 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
print(RF_Agnostic)

# ROC curve in training data
train_pred_votes_Agnostic <- predict(RF_Agnostic, newdata = predictor_data, type = "vote")
ROCTrain <- roc(usedTrainGroup, train_pred_votes_Agnostic[,2], plot = F, print.auc=TRUE, levels = c("Sensitive", "Resistant"), direction = "<", col="blue", lwd=2, grid=TRUE, ci = T)
ROCTrain
...
confusionTrain <- confusionMatrix(train_pred_Response_Agnostic, usedTrainGroup, positive = "Resistant")
confusionTrain
...
confusionTest <- confusionMatrix(RF_predictions_responses_Agnostic, usedTestGroup, positive = "Resistant")
confusionTest
...
## ROC curve and AUC
ROCTest <- roc(usedTestGroup, RF_predictions_votes_Agnostic[,2], plot = F, print.auc = TRUE, levels = c("Sensitive", "Resistant"), direction = "<", col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTest
...
GSE140494_Out_RF_IndvGenes_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE140494_Out_RF_IndvGenes_AgnosticPerformance, file = "./Objs/RF/GSE140494_Out_RF_IndvGenes_AgnosticPerformance.rda")
