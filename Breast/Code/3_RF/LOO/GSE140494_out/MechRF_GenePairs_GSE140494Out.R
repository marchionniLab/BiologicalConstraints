...
set.seed(333)
RF_Mechanistic_OnKTSP <- randomForest(x =predictor_data, y=usedTrainGroup, importance = TRUE, ntree = 500, mtry = 15 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
...
confusionTrain <- confusionMatrix(train_pred_responses_Mechanistic_OnKTSP, usedTrainGroup, positive = "Resistant")
confusionTrain
...
ROCTrain <- roc(usedTrainGroup, train_pred_votes_Mechanistic_OnKTSP[,2], plot = F, print.auc = TRUE, levels = c("Sensitive", "Resistant"), direction = "<", col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTrain
...
confusionTest <- confusionMatrix(RF_predictions_responses_Mechanistic_OnKTSP, usedTestGroup, positive = "Resistant")
confusionTest
...
ROCTest <- roc(usedTestGroup, RF_predictions_votes_Mechanistic_OnKTSP[,2], direction = "<", plot = F, print.auc = TRUE, levels = c("Sensitive", "Resistant"), col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTest
...
GSE140494_Out_RF_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE140494_Out_RF_MechPerformance, file = "./Objs/RF/GSE140494_Out_RF_MechPerformance.rda")
