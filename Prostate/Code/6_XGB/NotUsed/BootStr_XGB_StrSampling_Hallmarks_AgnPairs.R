...
set.seed(333)
bootobjectAgnostic <- boot(data= Data_train_Agnostic, statistic= XGBStrap, R= 1000, parallel = "multicore", ncpus = 12) 
...
set.seed(333)
bootobjectMech <- boot(data= Data_train_Mech, statistic= XGBStrap, R= 1000, parallel = "multicore", ncpus = 12) 
AUCs_XG_Mech <- bootobjectMech$t

colnames(AUCs_XG_Mech) <- c("AUC_Train", "AUC_Test", "N_ImportanVariables")

save(bootobjectAgnostic, bootobjectMech, file = "./Objs/XGB/XGBBootObjects_Hallmarks_AgnPairs.rda")

load("./Objs/XGB/XGBBootObjects_Hallmarks_AgnPairs.rda")
...
DiffAgnostic <- AUCs_XGB_Agnostic[, "AUC_Train"] - AUCs_XGB_Agnostic[, "AUC_Test"]
...
png("./Figs/XGB/XGB_BS_Hallmarks_AgnPairs.png", width = 3000, height = 1500, res = 300)
(AUCDistrHistTrain / AUCDistrHistTest) | (DiffHist_Agnostic / DiffHist_Mech) + plot_layout(widths = c(1, 1)) 
dev.off()
