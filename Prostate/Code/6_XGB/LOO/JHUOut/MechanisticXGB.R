...
###################
### Plot Curve
png("./Figs/XGB/LOO/CompareAUCggplot_JHUOut.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic XGB (JHU for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_XGB_JHUOut <- ggplot(dat_XGB, aes(d=Status, m=XGB_sum, color=XGB_type,
                                                 linetype = XGB_type)) +
...
### Plot
basicplot_XGB_JHUOut
### Close device
dev.off()

save(basicplot_XGB_JHUOut, file = "./Objs/XGB/LOO/BasicPlot_XGB_JHUOut.rda")
