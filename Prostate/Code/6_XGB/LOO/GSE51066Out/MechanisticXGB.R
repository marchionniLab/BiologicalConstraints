...
parameters <- list(
  ...
  subsample          = 0.3,           #0.4     # default = 1,   range: (0,1]
...
###################
### Plot Curve
png("./Figs/XGB/LOO/CompareAUCggplot_GSE51066Out.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic XGB (GSE51066 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_XGB_GSE51066Out <- ggplot(dat_XGB, aes(d=Status, m=XGB_sum, color=XGB_type,
                                                 linetype = XGB_type)) +
...
### Plot
basicplot_XGB_GSE51066Out
### Close device
dev.off()

save(basicplot_XGB_GSE51066Out, file = "./Objs/XGB/LOO/BasicPlot_XGB_GSE51066Out.rda")
