...
## Density plot
tiff(filename = "./Figs/Prostate_BS_AllModels_Density_new.tiff", width = 3000, height = 2000, res = 250)
...
png(filename = "./Figs/Prostate_BS_AllModels_Npairs.png", width = 3000, height = 1500, res = 200)
BS_AUC_ModelCompare <- ggplot(ModelCompare_KTSP_Npairs, aes(x = `N of output pairs`, y = `N of training features`, fill = `model type`, height = ..ndensity..)) + 
  ...
  scale_y_discrete(expand = c(0, 0)) +
  labs(title="The number of pairs returned by the K-TSPs model in the prostate cancer metastasis case") + 
  ylab("Model Type") +
  My_Theme +
  #scale_fill_manual(values = c("red3", "#78B7C5", "#3B9AB2")) +
  #scale_fill_jco() +
