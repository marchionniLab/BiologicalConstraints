...
AllModelCompare_Prostate_DiffNoFeat <- rbind(ModelCompare_KTSP_DiffNoFeat, ModelCompare_RF_DiffNoFeat, ModelCompare_SVM_DiffNoFeat, ModelCompare_XGB_DiffNoFeat)
...
AllModelCompare_Prostate_DiffNoFeat$modelType <- factor(AllModelCompare_Prostate_DiffNoFeat$modelType, levels = c("Agnostic_DEGs", "Agnostic_Pairs", "Mechanistic"))

levels(AllModelCompare_Prostate_DiffNoFeat$modelType) <- c("Agnostic DEGs", "Agnostic pairs", "Mechanistic")
...
AllModelCompare_Prostate_DiffNoFeat$NofFeatAgn[sel] <- "50_Pairs"
...
AllModelCompare_Prostate_DiffNoFeat$NofFeatAgn[sel2] <- "50_Pairs"

AllModelCompare_Prostate_DiffNoFeat$NofFeatAgn <- factor(AllModelCompare_Prostate_DiffNoFeat$NofFeatAgn, levels =  c("25_Pairs", "50_Pairs", "100_Pairs", "250_Pairs", "50_Genes", "100_Genes", "200_Genes", "500_Genes"))
levels(AllModelCompare_Prostate_DiffNoFeat$NofFeatAgn) <- c("25 Pairs", "50 Pairs", "100 Pairs", "250 Pairs", "50 Genes", "100 Genes", "200 Genes", "500 Genes")
...
## Density plot
png(filename = "./Figs/Prostate_BS_AllModels_Density_DiffNoFeatAndPairs_new.png", width = 3000, height = 1500, res = 200)
BS_AUC_ModelCompare <- ggplot(AllModelCompare_Prostate_DiffNoFeat, aes(x = AUC, y = modelType, fill = NofFeatAgn, height = ..ndensity..)) + 
  ...
  scale_x_continuous(limits = c(0.55, 0.8), breaks = seq(0.5, 0.9, by = 0.1), expand = c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
  labs(title="Mechanistic (50 pairs) vs agnostic (different N of features/pairs) performance at predicting prostate cancer metastasis in the testing data") + 
  ylab("Model Type") +
  My_Theme +
