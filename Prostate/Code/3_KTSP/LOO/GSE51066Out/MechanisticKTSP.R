...
ktspPredictorRes <- SWAP.Train.KTSP(
  usedTrainMat, usedTrainGroup, krange=11,
...
ROCTrain <- roc(usedTrainGroup, ktspStatsTrainRes$statistics, plot = F, print.thres=thr, print.thres.adj=c(0.01,1.25), print.auc=TRUE, ci = T, print.auc.col="black", levels = c("No_Mets", "Mets"), direction = "<", col="blue", lwd=2, grid=TRUE, main="Mechanistic KTSP performance in the training data")
ROCTrain 
...
## Plot curve
ROCTest <- roc(usedTestGroup, ktspStatsTestRes$statistics, plot = F, ci = T, print.auc=TRUE, print.auc.col="black", levels = c("No_Mets", "Mets"), direction = "<", col="blue", lwd=2, grid=TRUE, main= "Mechanistic KTSP using TF_targets Gns")
ROCTest 
...
GSE51066_Out_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE51066_Out_MechPerformance, file = "./Objs/KTSP/GSE51066_Out_MechPerformance.rda")
...
#################################################################
### Plot Curve
png("./Figs/KTSP/LOO/CompareAUCggplot_GSE51066Out.png",
...
### Plot
basicplot_KTSP_GSE51066Out <- ggplot(dat_KTSP, aes(d=Status, m=KTSP_sum, color=KTSP_type,
                                                   linetype = KTSP_type)) +
...
### Plot
basicplot_KTSP_GSE51066Out
### Close device
dev.off()

save(basicplot_KTSP_GSE51066Out, file = "./Objs/KTSP/LOO/BasicPlot_KTSP_GSE51066Out.rda")
