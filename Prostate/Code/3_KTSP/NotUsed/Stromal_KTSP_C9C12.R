...
load("./Objs/StromalPairs_C9C12.rda")
...
### Common genes
myTSPs <- as.matrix(StromalPairs_C9C12)
...
keep <- ktspPredictorRes$TSPs[,1] %in% myTSPs[,"c12"] & ktspPredictorRes$TSPs[,2] %in% myTSPs[,"c9"]
table(keep)
...
#################################################################
### Plot Curve
png("./Figs/KTSP/AUCggplot_SingleCellStroma_C9C12.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "Performance of the 11-TSPs model at predicting prostate cancer metastasis"
legendTitle <- paste("Mechanistic (", nrow(ktspPredictorRes$TSPs), " pairs)", "C9-C12")
### Plot
