...
### Common genes
myTSPs <- as.matrix(StromalPairs2)
...
keep <- ktspPredictorRes$TSPs[,1] %in% myTSPs[,1] & ktspPredictorRes$TSPs[,2] %in% myTSPs[,2]
table(keep)
...
#################################################################
### Plot Curve
png("./Figs/KTSP/AUCggplot_SingleCellStroma.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "Performance of the 9-TSPs model at predicting prostate cancer metastasis"
