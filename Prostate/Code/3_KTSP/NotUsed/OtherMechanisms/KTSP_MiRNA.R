...
Genes <- load("/Users/mohamedomar/Desktop/Genes/MiR_Genes.rda")
...
ktspPredictorRes <- SWAP.Train.KTSP(
  usedTrainMat, usedTrainGroup, krange=8,
...
## Threshold
#thr_test <- coords(roc(usedTestGroup, ktspStatsTestRes$statistics, levels = c("NoProgression", "Progression")),"best")["threshold"]
#thr_test

#####
## Print ROC curve local maximas
#coords(roc(usedTestGroup, ktspStatsTestRes$statistics, levels = c("NoProgression", "Progression")),"local maximas")
...
#####################
## Scatter plots
png(filename = "./Figs/KTSP/MechanisticKTSP_Train_ScatterPlot.png", width=3000, height=1500, res=300)
### Prepare ggplot
...
#####################
## Scatter plots
png(filename = "./Figs/KTSP/MechanisticKTSP_Test_ScatterPlot.png", width=3000, height=1500, res=300)
### Prepare ggplot
...
#################################################################
### Plot Curve
png("./Figs/KTSP/CompareAUCggplot_TF_MiRNA.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic (TF_MiRNAs) vs agnostic K-TSP"
...
basicplot_KTSP <- ggplot(dat_KTSP, aes(d=Status, m=KTSP_sum, color=KTSP_type,
                                       linetype = KTSP_type)) +
  geom_roc(increasing = F, cutoffs.at = seq(1,20,1)) +
