...
Genes <- read.delim("./Mets.xls")
Genes <- as.character(Genes$gene)

myTSPs2 <- t(combn(Genes,2))
...
ktspPredictorRes <- SWAP.Train.KTSP(
  usedTrainMat, usedTrainGroup, krange=18,
...
#####################
## Scatter plots
png(filename = "./Figs/KTSP/MechanisticKTSP_Train_ScatterPlot_HCMD.png", width=3000, height=1500, res=300)
### Prepare ggplot
...
#####################
## Scatter plots
png(filename = "./Figs/KTSP/MechanisticKTSP_Test_ScatterPlot_HCMD.png", width=3000, height=1500, res=300)
### Prepare ggplot
...
save(basicplot_KTSP_HCMD, file = "./Objs/KTSP/BasicPlot_KTSP_HCMD.rda")
