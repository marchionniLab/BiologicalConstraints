...
Genes_Bad <- read_tsv("./Data/Prognostic_Prostate_Bad.tsv")
Genes_Bad <- Genes_Bad$Gene

Genes_Good <- read_tsv("./Data/Prognostic_Prostate_Good.tsv")
Genes_Good <- Genes_Good$Gene  
  
myTSPs <- expand.grid(Genes_Bad, Genes_Good)
colnames(myTSPs) <- c("Bad_Prognosis", "Good_Prognosis")

# Check for duplicates
summary(myTSPs$Bad_Prognosis %in% myTSPs$Good_Prognosis)
...
#####################
## Scatter plots
png(filename = "./Figs/KTSP/MechanisticKTSP_Train_ScatterPlot_TFs.png", width=3000, height=1500, res=300)
### Prepare ggplot
...
#####################
## Scatter plots
png(filename = "./Figs/KTSP/MechanisticKTSP_Test_ScatterPlot_TFs.png", width=3000, height=1500, res=300)
### Prepare ggplot
...
#################################################################
### Plot Curve
png("./Figs/KTSP/CompareAUCggplot_TFs.png",
...
### Plot
basicplot_KTSP_TFs <- ggplot(dat_KTSP, aes(d=Status, m=KTSP_sum, color=KTSP_type,
                                           linetype = KTSP_type)) +
...
save(basicplot_KTSP_TFs, file = "./Objs/KTSP/BasicPlot_KTSP_TFs.rda")
...
TSP <- t(TSP)
write.csv(TSP, file = "./Objs/KTSP/TSP.csv",)
#OR
SWAP.PlotKTSP.Votes(ktspPredictorRes, as.matrix(usedTestMat), usedTestGroup)

SWAP.PlotKTSP.GenePairClassesBoxplot(genes = c("COX7B", "EZH2"), inputMat = as.matrix(usedTestMat), Groups = usedTestGroup, points = F, ordering  = "byClass")
#######################################################################
#### SAVE
save(list = ls(pattern = "^ktsp"), file = "./Objs/KTSP/mechanistic.ktspPredictor.rda")
