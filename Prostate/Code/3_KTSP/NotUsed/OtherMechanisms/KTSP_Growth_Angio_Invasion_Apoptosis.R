...
Growth <- load("/Users/mohamedomar/Desktop/Genes/MECHANISM/CancerHallmarks/CellGrowth/objs/growthPairs.v6.1.rda")
Invasion <- load("/Users/mohamedomar/Desktop/Genes/MECHANISM/CancerHallmarks/Invasion/objs/emtPairs.v6.1.rda")
Angiogenesis <- load("/Users/mohamedomar/Desktop/Genes/MECHANISM/CancerHallmarks/Angiogenesis/objs/angiogenesisPairs.v6.1.rda")
Apoptosis <- load("/Users/mohamedomar/Desktop/Genes/MECHANISM/CancerHallmarks/Apoptosis/objs/apoptosisPairs.v6.1.rda")

colnames(growthPairs) <- c("Gene1", "Gene2")
colnames(emtPairs) <- c("Gene1", "Gene2")
colnames(angiogenesisPairs) <- c("Gene1", "Gene2")
colnames(apoptosisPairs) <- c("Gene1", "Gene2")

myTSPs <- rbind(growthPairs, emtPairs, angiogenesisPairs, apoptosisPairs)
...
basicplot_KTSP_Combined <- ggplot(dat_KTSP, aes(d=Status, m=KTSP_sum, color=KTSP_type,
                                           linetype = KTSP_type)) +
  geom_roc(increasing = F, cutoffs.at = seq(1,10,1)) +
