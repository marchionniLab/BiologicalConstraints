...
NotchPairs[,c(1,2)] <- NotchPairs[,c(2,1)]

load("./Objs/MYCPairs.rda")
MycPairs[,c(1,2)] <- MycPairs[,c(2,1)]
...
ktspPredictorRes <- SWAP.Train.KTSP(
  usedTrainMat, usedTrainGroup, krange=58,
  FilterFunc = SWAP.Filter.Wilcoxon, featureNo=featNo, RestrictedPairs = myTSPs, disjoint = T)
...
save(KTSP_STATs_Train_Mechanistic, KTSP_STATs_Test_Mechanistic, file = "./Objs/KTSP/KTSP_STATs_Mechanistic_NotchAndMYC.rda")
