...
RF_Strap <- function(data, indices) {
  ...
  RF <- randomForest(usedTrainGroup~., data=d, importance = TRUE, ntree = 1001, mtry = 10 ,proximity=TRUE, na.action = na.omit)
...
set.seed(333)
bootobjectMech <- boot(data= DataMech_Train, statistic= RF_Strap, R= 1000) 
...
predictor_data_Train_Agnostic <- t(KTSP_STATs_Train_Agnostic)
predictor_data_Test_Agnostic <- t(KTSP_STATs_Test_Agnostic)
...
RF_Strap <- function(data, indices) {
  d <- data[indices, ] # allows boot to select sample
  RF <- randomForest(usedTrainGroup~., data=d, importance = TRUE, ntree = 1001, mtry = 5 ,proximity=TRUE, na.action = na.omit)
...
set.seed(333)
bootobjectAgnostic <- boot(data= DataAgnostic_Train, statistic= RF_Strap, R= 1000) 
...
save(bootobjectAgnostic, bootobjectMech, file = "./Objs/RF/RFBootObjects.rda")
...
DiffTrain <- AUCs_RF_Mech[, "AUC_Train"] - AUCs_RF_Agnostic[, "AUC_Train"]
quantile(DiffTrain, c(0.025, 0.975))
#colnames(Diff) <- "Diff"

## Calculate the difference and CI of the difference in the testing data
DiffTest <- AUCs_RF_Mech[, "AUC_Test"] - AUCs_RF_Agnostic[, "AUC_Test"]
...
DiffHist_Train <- ggplot(as.data.frame(DiffTrain), aes(DiffTrain, fill = "red")) + 
  geom_histogram(bins = 25) +
  scale_x_continuous(limits = c(-0.1, 0.1)) +
  labs(title="The difference in the training data") + My_Theme

DiffHist_Test <- ggplot(as.data.frame(DiffTest), aes(DiffTest, fill = "red")) + 
  geom_histogram(bins = 25) +
  scale_x_continuous(limits = c(-0.1, 0.1)) +
...
png("./Figs/RF/RF_BS_AUC_Compare_WithDiff.png", width = 3000, height = 1500, res = 300)
(AUC_Train_DistrHist / AUC_Test_DistrHist) | (DiffHist_Train / DiffHist_Test) + plot_layout(widths = c(1, 1)) | BarPlotImportance 
dev.off()
