...
set.seed(333)
RF_Agnostic <- randomForest(x =predictor_data, y=target, importance = TRUE, ntree = 1000, mtry = 32 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
print(RF_Agnostic)

# ROC curve in training data
train_pred_votes_Agnostic <- predict(RF_Agnostic, newdata = predictor_data, type = "vote")
ROCTrain <- roc(usedTrainGroup, train_pred_votes_Agnostic[,2], plot = F, print.auc=TRUE, levels = c("No_Mets", "Mets"), direction = "<", col="blue", lwd=2, grid=TRUE, ci = T)
ROCTrain
...
## ROC curve and AUC
ROCTest <- roc(usedTestGroup, RF_predictions_votes_Agnostic[,2], plot = F, print.auc = TRUE, levels = c("No_Mets", "Mets"), direction = "<", col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTest
...
GSE46691_Out_RF_IndvGenes_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE46691_Out_RF_IndvGenes_AgnosticPerformance, file = "./Objs/RF/GSE46691_Out_RF_IndvGenes_AgnosticPerformance.rda")
