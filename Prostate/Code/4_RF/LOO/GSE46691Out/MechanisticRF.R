...
## Load data
load("./Objs/KTSP/LOO/KTSP_STATs_Mechanistic_GSE46691Out.rda")
load("./Objs/LOO/MetastasisData_GSE46691Out.rda")
...
set.seed(333)
RF_Mechanistic_OnKTSP <- randomForest(x =predictor_data, y=usedTrainGroup, importance = TRUE, ntree = 500, mtry = 4, proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
...
confusionTrain <- confusionMatrix(train_pred_responses_Mechanistic_OnKTSP, usedTrainGroup, positive = "Mets")
confusionTrain
...
ROCTrain <- roc(usedTrainGroup, train_pred_votes_Mechanistic_OnKTSP[,2], plot = F, print.auc = TRUE, levels = c("No_Mets", "Mets"), direction = "<", col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTrain
...
confusionTest <- confusionMatrix(RF_predictions_responses_Mechanistic_OnKTSP, usedTestGroup, positive = "Mets")
confusionTest
...
ROCTest <- roc(usedTestGroup, RF_predictions_votes_Mechanistic_OnKTSP[,2], direction = "<", plot = F, print.auc = TRUE, levels = c("No_Mets", "Mets"), col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTest
...
GSE46691_Out_RF_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE46691_Out_RF_MechPerformance, file = "./Objs/RF/GSE46691_Out_RF_MechPerformance.rda")
