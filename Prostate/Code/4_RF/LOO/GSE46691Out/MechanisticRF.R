...
set.seed(333)
RF_Mechanistic_OnKTSP <- randomForest(x =predictor_data, y=usedTrainGroup, importance = TRUE, ntree = 501, mtry = 5 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
...
###################
### Plot Curve
png("./Figs/RF/LOO/CompareAUCggplot_GSE46691Out.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic RF (GSE46691 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_RF_GSE46691Out <- ggplot(dat_RF, aes(d=Status, m=RF_sum, color=RF_type,
                                               linetype = RF_type)) +
...
### Plot
basicplot_RF_GSE46691Out
### Close device
dev.off()


save(basicplot_RF_GSE46691Out, file = "./Objs/RF/LOO/BasicPlot_RF_GSE46691Out.rda")
