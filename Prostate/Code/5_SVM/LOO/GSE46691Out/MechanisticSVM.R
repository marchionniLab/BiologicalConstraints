...
## Cross study validation: GSE46691 Out
###########################################################################
...
Grid <- expand.grid(degree = 2, scale = 0.001, C = 0.5)
set.seed(333)
...
###################
### Plot Curve
png("./Figs/SVM/LOO/CompareAUCggplot_SVM_GSE46691Out.png",
    width=3000, height=3000, res=360)
### Color
myCol <- brewer.pal(3, "Dark2")[c(2,1)]
### Plot and legend titles
plotTitle <- "AUC mechanistic vs agnostic SVM (GSE46691 for validation)"
legendTitle <- "Mechanistic VS Agnostic"
### Plot
basicplot_SVM_GSE46691Out <- ggplot(dat_SVM, aes(d=Status, m=SVM_sum, color=SVM_type,
                                                 linetype = SVM_type)) +
...
### Plot
basicplot_SVM_GSE46691Out
### Close device
dev.off()

########################

save(basicplot_SVM_GSE46691Out, file = "./Objs/SVM/LOO/BasicPlot_SVM_GSE46691Out.rda")
