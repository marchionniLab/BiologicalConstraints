...
set.seed(333)
bootobjectAgnostic <- boot(data= Data_train_Agnostic, statistic= SVM_Strap, R= 1000, parallel = "multicore", ncpus = 12) 
...
set.seed(333)
bootobjectMech <- boot(data= Data_train_Mechanistic, statistic= SVM_Strap, R= 1000, parallel = "multicore", ncpus = 12) 

AUCs_SVM_Mech <- bootobjectMech$t
colnames(AUCs_SVM_Mech) <- c("AUC_Train", "AUC_Test", "N_ImportanVariables")

save(bootobjectAgnostic, bootobjectMech, file= "./Objs/SVM/SVMBootObjects_Hallmarks_AgnPairs.rda")

load("./Objs/SVM/SVMBootObjects_Hallmarks_AgnPairs.rda")
...
DiffAgnostic <- AUCs_SVM_Agnostic[, "AUC_Train"] - AUCs_SVM_Agnostic[, "AUC_Test"]
...
png("./Figs/SVM/SVM_BS_Hallmarks_AgnPairs.png", width = 3000, height = 1500, res = 300)
(AUC_Train_DistrHist / AUC_Test_DistrHist) | (DiffHist_Agnostic / DiffHist_Mech) + plot_layout(widths = c(1, 1)) #| BarPlotImportance
dev.off()
