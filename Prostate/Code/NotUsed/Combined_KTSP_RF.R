...
keepGns <- rownames(rf_importances)
...
ktspPredictorUnRes <- SWAP.Train.KTSP(usedTrainMat, usedTrainGroup, krange = c(1:50), FilterFunc = SWAP.Filter.Wilcoxon, featureNo = 500) # featureNo 400, allgenes
ktspPredictorUnRes
...
coords(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("No_Mets", "Mets"), direction = ">", ), transpose = TRUE ,"local maximas")

### Plot Curve: note that you must reorder the levels!!!
### ("good" goes first, "bad" goes second, the opposite of confusionMatrix)
roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, plot = TRUE, print.thres=thr, print.thres.adj=c(0.01,1.25), print.auc=TRUE, print.auc.col="black", levels = c("No_Mets", "Mets"), direction = ">", col="blue", lwd=2, grid=TRUE, main="Agnostic KTSP performance in the training data")
...
## Threshold
thr_test <- coords(roc(usedTestGroup, ktspStatsTestUnRes$statistics, levels = c("No_Mets", "Mets"), direction = ">",),"best", transpose = FALSE)["threshold"]
thr_test

## Print ROC curve local maximas
coords(roc(usedTestGroup, ktspStatsTestUnRes$statistics, levels = c("No_Mets", "Mets"), direction = ">",), transpose = TRUE, "local maximas")

## Plot curve
png("./Figs/Combined_KTSP_RF/ROC_Test.png", width = 2000, height = 2000, res = 300)
roc(usedTestGroup, ktspStatsTestUnRes$statistics, plot = TRUE, print.thres=thr_test, print.auc=TRUE, print.auc.col="black", levels = c("No_Mets", "Mets"), col="blue", lwd=2, grid=TRUE, auc = TRUE, ci = TRUE, percent = TRUE, main= "KTSP performance in the testing set")
dev.off()
