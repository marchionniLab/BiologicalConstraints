...
Agell_Sig <- c("AZGP1", "DPP4", "HGD", "MYBPC1", "PARP1", "PRKDC", "RNF19A", "SEC14L1", "SLPI", "TCEB1", "YWHAZ", "ZNF706")

#OncoDx_Genes <- c("BGN", "COL1A1", "SFRP4", "FLNC", "GSN", "TPM2", "GSTM2", "TPX2", "LAMB3", "FAM13C", "KLK2", "AZGP1", "SRD5A2", "DUSP1", "FOS")
save(Agell_Sig, file = "./Objs/Agell_Sig_Genes.rda")



summary(Agell_Sig %in% rownames(AllGenes))


# Keep only the stats of OncoDx genes (2 genes are absent)
Agell_Sig <- AllGenes[Agell_Sig, ]
#OncoDx <- na.omit(OncoDx)

# Replace the gene names with the OncoDx genes
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$posGeneNames <- c("PARP1", "PRKDC", "RNF19A", "SEC14L1", "SLPI", "TCEB1", "YWHAZ", "ZNF706")
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$negGeneNames <- c("AZGP1", "DPP4", "HGD", "MYBPC1")

Agell_Filter <- Prostate_metaanalysis$filterResults[[1]]
Agell_Filter_Summary <- summarizeFilterResults(metaObject = Prostate_metaanalysis, getMostRecentFilter(Prostate_metaanalysis))


## Create a summary ROC curve (Training data sets)
set.seed(333)
pooledROCPlot(metaObject = Prostate_metaanalysis, filterObject = Agell_Filter)
###########################################################################################
#############################################################################
...
# Validation dataset1
rocPlot(datasetObject = valDataSet1, filterObject = Agell_Filter)

# Validation dataset2
valDataSet2$keys <- rownames(val_expr2)
rocPlot(datasetObject = valDataSet2, filterObject = Agell_Filter)

# Validation dataset3
valDataSet3$keys <- rownames(val_expr3)
rocPlot(datasetObject = valDataSet3, filterObject = Agell_Filter)

# Validation dataset4
valDataSet4$keys <- rownames(val_expr4)
rocPlot(datasetObject = valDataSet4, filterObject = Agell_Filter)
...
###########################
# PRC plot
prcPlot(datasetObject = valDataSet1, filterObject = Agell_Filter)

# val dataset2
prcPlot(datasetObject = valDataSet2, filterObject = Agell_Filter)

# val dataset3
prcPlot(datasetObject = valDataSet3, filterObject = Agell_Filter)

# val dataset4
prcPlot(datasetObject = valDataSet4, filterObject = Agell_Filter)
...
##########################
# Violin plot
violinPlot(filterObject = Agell_Filter, datasetObject = valDataSet1, labelColumn = "Metastasis")

violinPlot(filterObject = Agell_Filter, datasetObject = valDataSet2, labelColumn = "Metastasis")

violinPlot(filterObject = Agell_Filter, datasetObject = valDataSet3, labelColumn = "Metastasis")

violinPlot(filterObject = Agell_Filter, datasetObject = valDataSet4, labelColumn = "Metastasis")
...
val_pheno1$score <- calculateScore(filterObject = Agell_Filter, datasetObject = valDataSet1)

# Val Dataset2
val_pheno2$score <- calculateScore(filterObject = Agell_Filter, datasetObject = valDataSet2)

# Val Dataset3
val_pheno3$score <- calculateScore(filterObject = Agell_Filter, datasetObject = valDataSet3)

# Val Dataset4
val_pheno4$score <- calculateScore(filterObject = Agell_Filter, datasetObject = valDataSet4)
...
set.seed(333)
png(filename = "./Objs/Meta/Agell_pooledROC_Testing.png", width = 2000, height = 2000, res = 300)
pooledROCPlot(metaObject = Meta_Test, filterObject = Agell_Filter)
dev.off()
...
## Kaplan-Meier curve
png(filename = "./Figs/Meta/Agell_KM_Survival_GSE116918.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data1),
           ...
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Agell signature Meta score in GSE116918", xlab = "Time in months", ylab = "Survival")
...
###################
png(filename = "./Figs/Meta/Agell_KM_Survival_JHU.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data3),
           ...
           pval.method = TRUE, 
           risk.table.y.text.col = TRUE,
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Agell signature Meta score in the JHU cohort", xlab = "Time in months", ylab = "Survival")
...
png(filename = "./Figs/Meta/Agell_CoxModel_GSE116918.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel1, data = Cox_data1, main = "Cox proportional hazards model using Agell signature in GSE116918")
dev.off()
...
png(filename = "./Figs/Meta/Agell_CoxModel_JHU.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel3, data = Cox_data3, main = "Cox proportional hazards model using Agell signature in JHU cohort")
dev.off()
