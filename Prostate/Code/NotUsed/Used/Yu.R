...
Yu_Sig <- c("CTSG", "CXCL12", "ACKR1", "EPHB6", "FST", "ITGB2", "KRT17", "NCKAP1L", "PRKG1", "PTGER3", "RLN1", "SNCA", "SOCS2", "WNT2")

#OncoDx_Genes <- c("BGN", "COL1A1", "SFRP4", "FLNC", "GSN", "TPM2", "GSTM2", "TPX2", "LAMB3", "FAM13C", "KLK2", "AZGP1", "SRD5A2", "DUSP1", "FOS")
save(Yu_Sig, file = "./Objs/Yu_Sig_Genes.rda")

summary(Yu_Sig %in% rownames(AllGenes))


Yu_Sig <- AllGenes[Yu_Sig, ]
#OncoDx <- na.omit(OncoDx)

# Replace the gene names with the OncoDx genes
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$posGeneNames <- "NA"
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$negGeneNames <- c("CTSG", "CXCL12", "ACKR1", "EPHB6", "FST", "ITGB2", "KRT17", "NCKAP1L", "PRKG1", "PTGER3", "RLN1", "SNCA", "SOCS2", "WNT2")

Yu_Filter <- Prostate_metaanalysis$filterResults[[1]]
Yu_Filter_Summary <- summarizeFilterResults(metaObject = Prostate_metaanalysis, getMostRecentFilter(Prostate_metaanalysis))


## Create a summary ROC curve (Training data sets)
set.seed(333)
pooledROCPlot(metaObject = Prostate_metaanalysis, filterObject = Yu_Filter)
###########################################################################################
#############################################################################
...
# Validation dataset1
rocPlot(datasetObject = valDataSet1, filterObject = Yu_Filter)

# Validation dataset2
valDataSet2$keys <- rownames(val_expr2)
rocPlot(datasetObject = valDataSet2, filterObject = Yu_Filter)

# Validation dataset3
valDataSet3$keys <- rownames(val_expr3)
rocPlot(datasetObject = valDataSet3, filterObject = Yu_Filter)

# Validation dataset4
valDataSet4$keys <- rownames(val_expr4)
rocPlot(datasetObject = valDataSet4, filterObject = Yu_Filter)
...
###########################
# PRC plot
prcPlot(datasetObject = valDataSet1, filterObject = Yu_Filter)

# val dataset2
prcPlot(datasetObject = valDataSet2, filterObject = Yu_Filter)

# val dataset3
prcPlot(datasetObject = valDataSet3, filterObject = Yu_Filter)

# val dataset4
prcPlot(datasetObject = valDataSet4, filterObject = Yu_Filter)
...
##########################
# Violin plot
violinPlot(filterObject = Yu_Filter, datasetObject = valDataSet1, labelColumn = "Metastasis")

violinPlot(filterObject = Yu_Filter, datasetObject = valDataSet2, labelColumn = "Metastasis")

violinPlot(filterObject = Yu_Filter, datasetObject = valDataSet3, labelColumn = "Metastasis")

violinPlot(filterObject = Yu_Filter, datasetObject = valDataSet4, labelColumn = "Metastasis")
...
val_pheno1$score <- calculateScore(filterObject = Yu_Filter, datasetObject = valDataSet1)

# Val Dataset2
val_pheno2$score <- calculateScore(filterObject = Yu_Filter, datasetObject = valDataSet2)

# Val Dataset3
val_pheno3$score <- calculateScore(filterObject = Yu_Filter, datasetObject = valDataSet3)

# Val Dataset4
val_pheno4$score <- calculateScore(filterObject = Yu_Filter, datasetObject = valDataSet4)
...
set.seed(333)
summaryROCPlot(metaObject = Meta_Test, filterObject = Yu_Filter)

set.seed(333)
png(filename = "./Objs/Meta/Yu_pooledROC_Testing.png", width = 2000, height = 2000, res = 300)
pooledROCPlot(metaObject = Meta_Test, filterObject = Yu_Filter)
dev.off()
...
## Kaplan-Meier curve
png(filename = "./Figs/Meta/Yu_KM_Survival_GSE116918.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data1),
           ...
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Yu signature Meta score in GSE116918", xlab = "Time in months", ylab = "Survival")
...
###################
png(filename = "./Figs/Meta/Yu_KM_Survival_JHU.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data3),
           ...
           pval.method = TRUE, 
           risk.table.y.text.col = TRUE,
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Yu signature Meta score in the JHU cohort", xlab = "Time in months", ylab = "Survival")
...
png(filename = "./Figs/Meta/Yu_CoxModel_GSE116918.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel1, data = Cox_data1, main = "Cox proportional hazards model using Yu signature in GSE116918")
dev.off()
...
png(filename = "./Figs/Meta/Yu_CoxModel_JHU.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel3, data = Cox_data3, main = "Cox proportional hazards model using Yu signature in JHU cohort")
dev.off()
