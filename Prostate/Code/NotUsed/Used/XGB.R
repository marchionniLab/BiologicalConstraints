...
### Common genes
#keepGns <- intersect(New_SignatureGenes, rownames(mixTrainMat))
...
xgb.agnostic <- xgb.train(parameters, DataTrain, nrounds = 500, watchlist,  early_stopping_rounds = 50, scale_pos_weight = NoMetastasis/Metasatsis)
...
#####################################################
## Second validation
## Predict in the Test data2
xgb_prob_test_agnostic2 <- predict(xgb.agnostic, DataTest2)

### Threshold
thr2 <- coords(roc(Test_label2, xgb_prob_test_agnostic2, levels = c("0", "1"), ), transpose = TRUE, "best")["threshold"]
thr2

### Print ROC curve local maximas
coords(roc(Test_label2, xgb_prob_test_agnostic2, levels = c("0", "1"), ), transpose = TRUE ,"local maximas")


#############################################

## Convert predicted probabilities to binary outcome
prediction_Test_agnostic2 <- as.numeric(xgb_prob_test_agnostic2 > thr2)
print(head(prediction_Test_agnostic2))

Test_label2 <- factor(Test_label2, levels = c(0,1))
prediction_Test_agnostic2 <- factor(prediction_Test_agnostic2, levels = c(0,1))

## Confusion matrix
CM_Test2 <- confusionMatrix(prediction_Test_agnostic2, Test_label2, positive = "1")
CM_Test2

# Calculate Matthews correlation coefficient
MCC_Test2 <- mcc(preds = prediction_Test_agnostic2, actuals = Test_label2)
MCC_Test2

## ROC curve and AUC
roc(Test_label2, xgb_prob_test_agnostic2, plot = F, print.auc = TRUE, levels = c("0", "1"), col = "black", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE, percent = TRUE, main = "XGB ROC curve in the testing cohort (Agnostic-Genes)")
