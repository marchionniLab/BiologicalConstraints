...
pheno7$Event <- as.numeric(pheno7$Metastasis)
pheno7$Event[pheno7$Event == 2] <- 0 

pheno7$Time <- as.numeric(pheno7$met_time)

Phenotype <- pheno7


# Keep only the Good AND Bad Genes (Associated with no metastasis // Metastasis)
exprUP <- expr7[rownames(expr7)%in%PositiveGenes, ]
exprDown <- expr7[rownames(expr7)%in%NegativeGenes, ]
...
SplotUp <- arrange_ggsurvplots(Up_PlotList, title = "Survival plots of up-regulated genes (JHU)",ncol = 2, nrow = 5)
ggsave("./Figs/Meta/UpGenes_JHU.pdf", SplotUp, width = 40, height = 30, units = "cm")
...
SplotDown <- arrange_ggsurvplots(Down_PlotList, title = "Survival plots of down-regulated genes (JHU)", ncol = 3, nrow = 6)
ggsave("./Figs/Meta/DownGenes_JHU.pdf", SplotDown, width = 40, height = 30, units = "cm")
