...
Varambally_Sig <- c("AURKA", "EZH2", "NUP62", "TMPO", "RALA", "UBE2I", "XPO1", "CDKN2A", "MSH2", "VTI1B", "UBA2", "SMARCC2", "TRIM28", "OCLN", "MCM6", "RAN", "MAPK9", "ILF3", "CSK", "HSD17B10", "STMN1", "UHRF1", "KPNA2", "CDK4", "MCM2", "RAB27A", "NEXN", "KLK3", "REG3A", "PRSS8", "MYLK", "MAP2K4", "TRAF3IP2", "PPP1R12A", "LIMK1", "CSRP1", "ITGA5", "FLNB", "CASP7", "MMP23B", "ACTA1", "DLGAP1", "MAPKAPK3", "BIRC2", "CAV2", "TGFB1I1", "PXN", "AMACR", "CRYAB", "CFLAR")

#OncoDx_Genes <- c("BGN", "COL1A1", "SFRP4", "FLNC", "GSN", "TPM2", "GSTM2", "TPX2", "LAMB3", "FAM13C", "KLK2", "AZGP1", "SRD5A2", "DUSP1", "FOS")
save(Varambally_Sig, file = "./Objs/Varambally_Sig_Genes.rda")



summary(Varambally_Sig %in% rownames(AllGenes))


# Keep only the stats of OncoDx genes (2 genes are absent)
Varambally_Sig <- AllGenes[Varambally_Sig, ]
#OncoDx <- na.omit(OncoDx)

# Replace the gene names with the OncoDx genes
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$posGeneNames <- c("AURKA", "EZH2", "NUP62", "TMPO", "RALA", "UBE2I", "XPO1", "CDKN2A", "MSH2", "VTI1B", "UBA2", "SMARCC2", "TRIM28", "OCLN", "MCM6", "RAN", "MAPK9", "ILF3", "CSK", "HSD17B10", "STMN1", "UHRF1", "KPNA2", "CDK4", "MCM2")
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$negGeneNames <- c("RAB27A", "NEXN", "KLK3", "REG3A", "PRSS8", "MYLK", "MAP2K4", "TRAF3IP2", "PPP1R12A", "LIMK1", "CSRP1", "ITGA5", "FLNB", "CASP7", "MMP23B", "ACTA1", "DLGAP1", "MAPKAPK3", "BIRC2", "CAV2", "TGFB1I1", "PXN", "AMACR", "CRYAB", "CFLAR")

Varambally_Filter <- Prostate_metaanalysis$filterResults[[1]]
Varambally_Filter_Summary <- summarizeFilterResults(metaObject = Prostate_metaanalysis, getMostRecentFilter(Prostate_metaanalysis))


## Create a summary ROC curve (Training data sets)
set.seed(333)
pooledROCPlot(metaObject = Prostate_metaanalysis, filterObject = Varambally_Filter)
###########################################################################################
#############################################################################
...
# Validation dataset1
rocPlot(datasetObject = valDataSet1, filterObject = Varambally_Filter)

# Validation dataset2
valDataSet2$keys <- rownames(val_expr2)
rocPlot(datasetObject = valDataSet2, filterObject = Varambally_Filter)

# Validation dataset3
valDataSet3$keys <- rownames(val_expr3)
rocPlot(datasetObject = valDataSet3, filterObject = Varambally_Filter)

# Validation dataset4
valDataSet4$keys <- rownames(val_expr4)
rocPlot(datasetObject = valDataSet4, filterObject = Varambally_Filter)
...
###########################
# PRC plot
prcPlot(datasetObject = valDataSet1, filterObject = Varambally_Filter)

# val dataset2
prcPlot(datasetObject = valDataSet2, filterObject = Varambally_Filter)

# val dataset3
prcPlot(datasetObject = valDataSet3, filterObject = Varambally_Filter)

# val dataset4
prcPlot(datasetObject = valDataSet4, filterObject = Varambally_Filter)
...
##########################
# Violin plot
violinPlot(filterObject = Varambally_Filter, datasetObject = valDataSet1, labelColumn = "Metastasis")

violinPlot(filterObject = Varambally_Filter, datasetObject = valDataSet2, labelColumn = "Metastasis")

violinPlot(filterObject = Varambally_Filter, datasetObject = valDataSet3, labelColumn = "Metastasis")

violinPlot(filterObject = Varambally_Filter, datasetObject = valDataSet4, labelColumn = "Metastasis")
...
val_pheno1$score <- calculateScore(filterObject = Varambally_Filter, datasetObject = valDataSet1)

# Val Dataset2
val_pheno2$score <- calculateScore(filterObject = Varambally_Filter, datasetObject = valDataSet2)

# Val Dataset3
val_pheno3$score <- calculateScore(filterObject = Varambally_Filter, datasetObject = valDataSet3)

# Val Dataset4
val_pheno4$score <- calculateScore(filterObject = Varambally_Filter, datasetObject = valDataSet4)
...
set.seed(333)
summaryROCPlot(metaObject = Meta_Test, filterObject = Agell_Filter)

set.seed(333)
png(filename = "./Objs/Meta/Varambally_pooledROC_Testing.png", width = 2000, height = 2000, res = 300)
pooledROCPlot(metaObject = Meta_Test, filterObject = Varambally_Filter)
dev.off()
...
## Kaplan-Meier curve
png(filename = "./Figs/Meta/Varambally_KM_Survival_GSE116918.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data1),
           ...
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Varambally signature Meta score in GSE116918", xlab = "Time in months", ylab = "Survival")
...
###################
png(filename = "./Figs/Meta/Varambally_KM_Survival_JHU.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data3),
           ...
           pval.method = TRUE, 
           risk.table.y.text.col = TRUE,
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Varambally signature Meta score in the JHU cohort", xlab = "Time in months", ylab = "Survival")
...
png(filename = "./Figs/Meta/Varambally_CoxModel_GSE116918.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel1, data = Cox_data1, main = "Cox proportional hazards model using Varambally signature in GSE116918")
dev.off()
...
png(filename = "./Figs/Meta/Varambally_CoxModel_JHU.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel3, data = Cox_data3, main = "Cox proportional hazards model using Varambally signature in JHU cohort")
dev.off()
