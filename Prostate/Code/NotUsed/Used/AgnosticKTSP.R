...
### Threshold
thr <- coords(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("No_Mets", "Mets")), transpose = TRUE, "best")["threshold"]
thr

### Print ROC curve local maximas
coords(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("No_Mets", "Mets")), transpose = TRUE ,"local maximas")

### Plot Curve: note that you must reorder the levels!!!
### ("good" goes first, "bad" goes second, the opposite of confusionMatrix)
roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, plot = F, print.thres=thr, print.thres.adj=c(0.01,1.25), print.auc=TRUE, print.auc.col="black", levels = c("No_Mets", "Mets"), col="blue", lwd=2, grid=TRUE, main="Agnostic KTSP performance in the training data")
...
## Threshold
thr_test <- coords(roc(usedTestGroup, ktspStatsTestUnRes$statistics, levels = c("No_Mets", "Mets")),"best", transpose = FALSE)["threshold"]
thr_test

## Print ROC curve local maximas
coords(roc(usedTestGroup, ktspStatsTestUnRes$statistics, levels = c("No_Mets", "Mets")), transpose = TRUE, "local maximas")

## Plot curve
roc(usedTestGroup, ktspStatsTestUnRes$statistics, plot = F, print.thres=thr_test, print.auc=TRUE, print.auc.col="black", levels = c("No_Mets", "Mets"), col="blue", lwd=2, grid=TRUE, auc = TRUE, ci = TRUE, percent = TRUE, main= "Agnostic KTSP performance in the testing set")

### Get predictions based on best threshold from ROC curve
usedTestPredictionUnRes <- SWAP.KTSP.Classify(usedTestMat, ktspPredictorUnRes, DecisionFunc = function(x) sum(x) > thr_test)

### Resubstitution performance in the Test set
confusionMatrix(usedTestPredictionUnRes, usedTestGroup, positive = "Mets")

mcc(preds = usedTestPredictionUnRes, actuals = usedTestGroup)
...
########################
## Make paired boxplot
png("./Figs/Unrestricted/Agnostic.trainKTSPexprs.png", width = 3000, height = 1500, res = 200)
...
########################
## Make paired boxplot
png("./Figs/UnRestricted/Agnostic.testKTSPexprs.png", width = 3000, height = 1500, res = 200)
...
#######################################################################
#### SAVE
save(list = ls(pattern = "^ktsp"), file = "./Objs/UnRestricted/Agnostic.ktspPredictor.rda")
