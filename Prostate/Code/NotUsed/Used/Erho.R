...
## The Erho genes (16/22 genes)
Erho_Sig <- c("LASP1", "IQGAP3", "NFIB", "S1PR4", "THBS2", "ANO7", "PCDH7", "MYBPC1", "EPPK1", "PBX1", "NUSAP1", "ZWILCH", "UBE2C", "CAMK2N1", "RABGAP1", "TNFRSF19")
...
summary(Erho_Sig %in% rownames(AllGenes))


Erho_Sig <- AllGenes[Erho_Sig, ]
#OncoDx <- na.omit(OncoDx)

# Replace the gene names with the OncoDx genes
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$posGeneNames <- c("LASP1", "IQGAP3", "THBS2", "EPPK1", "PBX1", "NUSAP1", "ZWILCH", "UBE2C", "CAMK2N1")
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$negGeneNames <- c("NFIB", "S1PR4", "ANO7", "PCDH7", "MYBPC1", "RABGAP1", "TNFRSF19")

Erho_filter <- Prostate_metaanalysis$filterResults[[1]]
Erho_Filter_Summary <- summarizeFilterResults(metaObject = Prostate_metaanalysis, getMostRecentFilter(Prostate_metaanalysis))


## Create a summary ROC curve (Training data sets)
set.seed(333)
pooledROCPlot(metaObject = Prostate_metaanalysis, filterObject = Erho_filter)
###########################################################################################
#############################################################################
...
# Validation dataset1
rocPlot(datasetObject = valDataSet1, filterObject = Erho_filter)

# Validation dataset2
valDataSet2$keys <- rownames(val_expr2)
rocPlot(datasetObject = valDataSet2, filterObject = Erho_filter)

# Validation dataset3
valDataSet3$keys <- rownames(val_expr3)
rocPlot(datasetObject = valDataSet3, filterObject = Erho_filter)

# Validation dataset4
valDataSet4$keys <- rownames(val_expr4)
rocPlot(datasetObject = valDataSet4, filterObject = Erho_filter)
...
###########################
# PRC plot
prcPlot(datasetObject = valDataSet1, filterObject = Erho_filter)

# val dataset2
prcPlot(datasetObject = valDataSet2, filterObject = Erho_filter)

# val dataset3
prcPlot(datasetObject = valDataSet3, filterObject = Erho_filter)

# val dataset4
prcPlot(datasetObject = valDataSet4, filterObject = Erho_filter)
...
##########################
# Violin plot
violinPlot(filterObject = Erho_filter, datasetObject = valDataSet1, labelColumn = "Metastasis")

violinPlot(filterObject = Erho_filter, datasetObject = valDataSet2, labelColumn = "Metastasis")

violinPlot(filterObject = Erho_filter, datasetObject = valDataSet3, labelColumn = "Metastasis")

violinPlot(filterObject = Erho_filter, datasetObject = valDataSet4, labelColumn = "Metastasis")
...
val_pheno1$score <- calculateScore(filterObject = Erho_filter, datasetObject = valDataSet1)

# Val Dataset2
val_pheno2$score <- calculateScore(filterObject = Erho_filter, datasetObject = valDataSet2)

# Val Dataset3
val_pheno3$score <- calculateScore(filterObject = Erho_filter, datasetObject = valDataSet3)

# Val Dataset4
val_pheno4$score <- calculateScore(filterObject = Erho_filter, datasetObject = valDataSet4)
...
set.seed(333)
summaryROCPlot(metaObject = Meta_Test, filterObject = Erho_filter)

set.seed(333)
png(filename = "./Objs/Meta/Erho_pooledROC_Testing.png", width = 2000, height = 2000, res = 300)
pooledROCPlot(metaObject = Meta_Test, filterObject = Erho_filter)
dev.off()
...
## Kaplan-Meier curve
png(filename = "./Figs/Meta/Erho_KM_Survival_GSE116918.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data1),
           ...
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Erho signature Meta score in GSE116918", xlab = "Time in months", ylab = "Survival")
...
###################
png(filename = "./Figs/Meta/Erho_KM_Survival_JHU.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data3),
           ...
           pval.method = TRUE, 
           risk.table.y.text.col = TRUE,
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Erho signature Meta score in the JHU cohort", xlab = "Time in months", ylab = "Survival")
...
png(filename = "./Figs/Meta/Erho_CoxModel_GSE116918.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel1, data = Cox_data1, main = "Cox proportional hazards model using Erho signature in GSE116918")
dev.off()
...
png(filename = "./Figs/Meta/Erho_CoxModel_JHU.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel3, data = Cox_data3, main = "Cox proportional hazards model using Erho signature in JHU cohort")
dev.off()
