...
Wu_Sig <- c("ACTR3B", "APOC1", "ATP8A1", "ADIRF", "CDK1", "CDON", "DEGS1", "SYNM", "DPP4", "F12", "FEV", "GATA3", "GSTM3", "HIST1H3H", "HOXC4", "TMEM132A", "IGSF1", "IGSF6", "INHBA", "KRT15", "LDHB", "KIF18B", "NCAPG2", "MAOA", "MT1F", "OIP5", "PPP3CB", "QPRT", "TPX2")

#OncoDx_Genes <- c("BGN", "COL1A1", "SFRP4", "FLNC", "GSN", "TPM2", "GSTM2", "TPX2", "LAMB3", "FAM13C", "KLK2", "AZGP1", "SRD5A2", "DUSP1", "FOS")
save(Wu_Sig, file = "./Objs/Wu_Sig_Genes.rda")

summary(Wu_Sig %in% rownames(AllGenes))


Wu_Sig <- AllGenes[Wu_Sig, ]
#OncoDx <- na.omit(OncoDx)

# Replace the gene names with the OncoDx genes
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$posGeneNames <- c("APOC1", "ATP8A1", "DEGS1", "F12", "TMEM132A", "INHBA", "NCAPG2", "MAOA", "OIP5", "QPRT", "TPX2")
Prostate_metaanalysis$filterResults$FDR0.1_es0_nStudies6_looaFALSE_hetero0$negGeneNames <- c("ACTR3B", "ADIRF", "CDK1", "CDON", "SYNM", "DPP4", "FEV", "GATA3", "GSTM3", "HIST1H3H", "HOXC4", "IGSF1", "IGSF6", "KRT15", "LDHB", "KIF18B", "MT1F", "PPP3CB")

Wu_Filter <- Prostate_metaanalysis$filterResults[[1]]
Wu_Filter_Summary <- summarizeFilterResults(metaObject = Prostate_metaanalysis, getMostRecentFilter(Prostate_metaanalysis))


## Create a summary ROC curve (Training data sets)
set.seed(333)
pooledROCPlot(metaObject = Prostate_metaanalysis, filterObject = Wu_Filter)
###########################################################################################
#############################################################################
...
# Validation dataset1
rocPlot(datasetObject = valDataSet1, filterObject = Wu_Filter)

# Validation dataset2
valDataSet2$keys <- rownames(val_expr2)
rocPlot(datasetObject = valDataSet2, filterObject = Wu_Filter)

# Validation dataset3
valDataSet3$keys <- rownames(val_expr3)
rocPlot(datasetObject = valDataSet3, filterObject = Wu_Filter)

# Validation dataset4
valDataSet4$keys <- rownames(val_expr4)
rocPlot(datasetObject = valDataSet4, filterObject = Wu_Filter)
...
###########################
# PRC plot
prcPlot(datasetObject = valDataSet1, filterObject = Wu_Filter)

# val dataset2
prcPlot(datasetObject = valDataSet2, filterObject = Wu_Filter)

# val dataset3
prcPlot(datasetObject = valDataSet3, filterObject = Wu_Filter)

# val dataset4
prcPlot(datasetObject = valDataSet4, filterObject = Wu_Filter)
...
##########################
# Violin plot
violinPlot(filterObject = Wu_Filter, datasetObject = valDataSet1, labelColumn = "Metastasis")

violinPlot(filterObject = Wu_Filter, datasetObject = valDataSet2, labelColumn = "Metastasis")

violinPlot(filterObject = Wu_Filter, datasetObject = valDataSet3, labelColumn = "Metastasis")

violinPlot(filterObject = Wu_Filter, datasetObject = valDataSet4, labelColumn = "Metastasis")
...
val_pheno1$score <- calculateScore(filterObject = Wu_Filter, datasetObject = valDataSet1)

# Val Dataset2
val_pheno2$score <- calculateScore(filterObject = Wu_Filter, datasetObject = valDataSet2)

# Val Dataset3
val_pheno3$score <- calculateScore(filterObject = Wu_Filter, datasetObject = valDataSet3)

# Val Dataset4
val_pheno4$score <- calculateScore(filterObject = Wu_Filter, datasetObject = valDataSet4)
...
set.seed(333)
summaryROCPlot(metaObject = Meta_Test, filterObject = Wu_Filter)

set.seed(333)
png(filename = "./Objs/Meta/Wu_pooledROC_Testing.png", width = 2000, height = 2000, res = 300)
pooledROCPlot(metaObject = Meta_Test, filterObject = Wu_Filter)
dev.off()
...
## Kaplan-Meier curve
png(filename = "./Figs/Meta/Wu_KM_Survival_GSE116918.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data1),
           ...
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Wu signature Meta score in GSE116918", xlab = "Time in months", ylab = "Survival")
...
###################
png(filename = "./Figs/Meta/Wu_KM_Survival_JHU.png", width = 2000, height = 2000, res = 300)
ggsurvplot(survfit(Surv(Time, Event) ~ Meta_Score, data = surv_data3),
           ...
           pval.method = TRUE, 
           risk.table.y.text.col = TRUE,
           risk.table.y.text = TRUE, palette = c("green", "red"), title = "Survival by the Wu signature Meta score in the JHU cohort", xlab = "Time in months", ylab = "Survival")
...
png(filename = "./Figs/Meta/Wu_CoxModel_GSE116918.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel1, data = Cox_data1, main = "Cox proportional hazards model using Wu signature in GSE116918")
dev.off()
...
png(filename = "./Figs/Meta/Wu_CoxModel_JHU.png", width = 3000, height = 2000, res =400)
ggforest(CoxModel3, data = Cox_data3, main = "Cox proportional hazards model using Wu signature in JHU cohort")
dev.off()
