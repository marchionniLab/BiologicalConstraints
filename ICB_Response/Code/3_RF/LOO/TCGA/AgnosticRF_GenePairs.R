...
set.seed(333)
RF_Agnostic_OnKTSP <- randomForest(x =predictor_data, y=usedTrainGroup, importance = TRUE, ntree = 50, mtry = 20 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
RF_Agnostic_OnKTSP
...
TCGA_Out_RF_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(TCGA_Out_RF_AgnosticPerformance, file = "./Objs/RF/TCGA_Out_RF_AgnosticPerformance.rda")
