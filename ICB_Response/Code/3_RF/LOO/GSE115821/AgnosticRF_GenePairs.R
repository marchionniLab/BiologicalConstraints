...
set.seed(333)
RF_Agnostic_OnKTSP <- randomForest(x =predictor_data, y=usedTrainGroup, importance = TRUE, ntree = 50, mtry = 10 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
RF_Agnostic_OnKTSP
...
confusionTrain <- confusionMatrix(train_pred_responses_Agnostic_OnKTSP, usedTrainGroup, positive = "R")
confusionTrain
...
ROCTrain <- roc(usedTrainGroup, train_pred_votes_Agnostic_OnKTSP[,2], plot = F, print.auc = TRUE, levels = c("NR", "R"), direction = "<", col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTrain
...
confusionTest <- confusionMatrix(RF_predictions_responses_Agnostic_OnKTSP, usedTestGroup, positive = "R")
confusionTest
...
ROCTest <- roc(usedTestGroup, RF_predictions_votes_Agnostic_OnKTSP[,2], plot = F, print.auc = TRUE, levels = c("NR", "R"), direction = "<", col = "blue", lwd = 2, grid = TRUE, auc = TRUE, ci = TRUE)
ROCTest
...
GSE115821_Out_RF_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE115821_Out_RF_AgnosticPerformance, file = "./Objs/RF/GSE115821_Out_RF_AgnosticPerformance.rda")
