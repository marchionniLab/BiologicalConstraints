...
AllModelCompare_Breast_DiffNoFeat$NofFeatAgn[sel] <- "27_Pairs"

sel2 <- which(AllModelCompare_Breast_DiffNoFeat$modelType == "Mechanistic")
AllModelCompare_Breast_DiffNoFeat$NofFeatAgn[sel2] <- "27_Pairs"

table(AllModelCompare_Breast_DiffNoFeat$modelType, AllModelCompare_Breast_DiffNoFeat$NofFeatAgn)

AllModelCompare_Breast_DiffNoFeat$NofFeatAgn <- factor(AllModelCompare_Breast_DiffNoFeat$NofFeatAgn, levels =  c("25_Pairs", "27_Pairs", "50_Pairs", "100_Pairs", "250_Pairs", "50_Genes", "100_Genes", "200_Genes", "500_Genes"))
levels(AllModelCompare_Breast_DiffNoFeat$NofFeatAgn) <- c("25 Pairs", '27 Pairs', "50 Pairs", "100 Pairs", "250 Pairs", "50 Genes", "100 Genes", "200 Genes", "500 Genes")
...
BS_AUC_ModelCompare <- ggplot(AllModelCompare_Breast_DiffNoFeat, aes(x = AUC, y = modelType, fill = NofFeatAgn, height = ..ndensity..)) + 
  ...
  scale_x_continuous(limits = c(0.35, 0.90), breaks = seq(0.3, 0.9, by = 0.1), expand = c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
  labs(title="Mechanistic (27 pairs) vs agnostic (different N of features and pairs) performance at predicting the response to ICIs in the testing data") + 
  ylab("Model Type") +
  My_Theme +
...
BS_AUC_ModelCompare <- ggplot(ModelCompare_SVM_DiffNoFeat, aes(x = AUC, y = modelType, fill = NofFeatAgn, height = ..ndensity..)) + 
  geom_density_ridges(stat = "density", alpha = 0.9, bw = 0.8, adjust= 0.01, scale=1.2) +
  scale_x_continuous(limits = c(0.25, 1), breaks = seq(0.3, 0.9, by = 0.1), expand = c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
