...
## Agnostic
load("./Objs/KTSP/KTSP_STATs_Agnostic_TCGAOut.rda")
...
Grid <- expand.grid(degree = 1, scale = 0.01, C = 2)
set.seed(333)
...
ROCTrain <- roc(usedTrainGroup, train_pred_prob_svmPoly_agnostic_OnKTSP[,2], plot = F, print.auc=TRUE, ci = T, levels = c("NR", "R"), direction = "<", col="blue", lwd=2, grid=TRUE, main = "ROC Test SVM Poly (agnostic)")
ROCTrain

# The best threshold
thr <- coords(roc(usedTrainGroup, train_pred_prob_svmPoly_agnostic_OnKTSP[,2], levels = c("NR", "R"), direction = "<"), "best")["threshold"]
thr <- thr$threshold
thr

# Predict classes according to the best threshold
train_pred_classes_svmPoly_agnostic_OnKTSP <- ifelse(train_pred_prob_svmPoly_agnostic_OnKTSP[,2] > thr, "R", "NR")
table(train_pred_classes_svmPoly_agnostic_OnKTSP)
# Convert to factor
train_pred_classes_svmPoly_agnostic_OnKTSP <- factor(train_pred_classes_svmPoly_agnostic_OnKTSP, levels = c("NR", "R"))

## Creat a confusion matrix (the predictions against the true classes)
Confusion_train_svmPoly_OnKTSP <- confusionMatrix(train_pred_classes_svmPoly_agnostic_OnKTSP, usedTrainGroup, positive = "R")
...
ROCTest <- roc(usedTestGroup, test_pred_prob_svmPoly_agnostic_OnKTSP[,2], plot = F, print.auc=TRUE, ci = T, levels = c("NR", "R"), direction = "<", col="blue", lwd=2, grid=TRUE, main = "ROC Test SVM Poly (agnostic)")
ROCTest

# Predict classes according to the best threshold
test_pred_classes_svmPoly_agnostic_OnKTSP <- ifelse(test_pred_prob_svmPoly_agnostic_OnKTSP[,2] > thr, "R", "NR")
table(test_pred_classes_svmPoly_agnostic_OnKTSP)
# Convert to factor
test_pred_classes_svmPoly_agnostic_OnKTSP <- factor(test_pred_classes_svmPoly_agnostic_OnKTSP, levels = c("NR", "R"))

## Creat a confusion matrix (the predictions against the true classes)
Confusion_test_svmPoly_OnKTSP <- confusionMatrix(test_pred_classes_svmPoly_agnostic_OnKTSP, usedTestGroup, positive = "R")
...
TCGA_Out_SVM_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(TCGA_Out_SVM_AgnosticPerformance, file = "./Objs/SVM/TCGA_Out_SVM_AgnosticPerformance.rda")
