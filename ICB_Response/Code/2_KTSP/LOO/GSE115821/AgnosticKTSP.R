...
## Threshold
thr <- coords(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("NR", "R"), direction = "<"), "best")["threshold"]
thr

## Print ROC curve local maximas
coords(roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, levels = c("NR", "R"), direction = "<"), "local maximas")

### Plot Curve: reorder the levels ("good" goes first, "bad" goes second, the opposite of confusionMatrix)
ROCTrain <- roc(usedTrainGroup, ktspStatsTrainUnRes$statistics, plot = FALSE, ci = TRUE, print.auc = TRUE, print.auc.col="black", levels = c("NR", "R"), direction = "<", col = "blue", lwd = 2, grid = TRUE, main = "Agnostic KTSP ROC Train")
ROCTrain
...
confusionTrain <- confusionMatrix(usedTrainPredictionUnRes, usedTrainGroup, positive = "R")
confusionTrain
...
# plot the curve
ROCTest <- roc(usedTestGroup, ktspStatsTestUnRes$statistics, plot = F, print.auc=TRUE, ci= T, print.auc.col="black", levels = c("NR", "R"), direction = "<", col= "blue", lwd=2, grid = TRUE, main = "Agnostic KTSP ROC Test")
ROCTest
...
confusionTest <- confusionMatrix(usedTestPredictionUnRes, usedTestGroup, positive = "R", mode = "everything")
confusionTest
...
GSE115821_Out_AgnosticPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE115821_Out_AgnosticPerformance, file = "./Objs/KTSP/GSE115821_Out_AgnosticPerformance.rda")
