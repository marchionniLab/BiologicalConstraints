...
ktspPredictorRes <- SWAP.Train.KTSP(
  usedTrainMat, usedTrainGroup, krange=10,
...
### Threshold
thr <- coords(roc(usedTrainGroup, ktspStatsTrainRes$statistics, levels = c("NR", "R"), direction = "<" ), "best")["threshold"]
thr

### Print ROC curve local maximas
coords(roc(usedTrainGroup, ktspStatsTrainRes$statistics, levels = c("NR", "R"), direction = "<" ), "local maximas")

### Plot Curve: note that you must reorder the levels!!!
### ("good" goes first, "bad" goes second, the opposite of confusionMatrix)
ROCTrain <- roc(usedTrainGroup, ktspStatsTrainRes$statistics, plot = F, print.thres=thr, print.thres.adj=c(0.01,1.25), print.auc=TRUE, ci = T, print.auc.col="black", levels = c("NR", "R"), direction = "<", col="blue", lwd=2, grid=TRUE, main="Mechanistic KTSP performance in the training data")
ROCTrain
...
confusionMatrix(usedTrainPredictionRes, usedTrainGroup, positive = "R", mode = "everything")

MCC_Mechanistic_Train <- mltools::mcc(pred = usedTrainPredictionRes, actuals = usedTrainGroup)
MCC_Mechanistic_Train

confusionTrain <- confusionMatrix(usedTrainPredictionRes, usedTrainGroup, positive = "R", mode = "everything")
confusionTrain
...
## Plot curve
ROCTest <- roc(usedTestGroup, ktspStatsTestRes$statistics, plot = F, print.auc=TRUE, ci = T, print.auc.col="black", levels = c("NR", "R"), direction = "<", col="blue", lwd=2, grid=TRUE, main= "Mechanistic KTSP using TF_MiR Gns")
ROCTest
...
confusionTest <- confusionMatrix(usedTestPredictionRes, usedTestGroup, positive = "R", mode = "everything")
confusionTest
...
GSE78220_Out_MechPerformance <- cbind(TrainPerf, TestPerf)

# Save
save(GSE78220_Out_MechPerformance, file = "./Objs/KTSP/GSE78220_Out_MechPerformance.rda")
